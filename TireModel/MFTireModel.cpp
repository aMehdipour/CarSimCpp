...
void MFTireModel::computeLateralForce(double& Fx,
                                      double& Fy,
                                      double u,
                                      double v,
                                      double camberAngle,
    ...
    double SVX     = normalLoad * (coeffs_.PVX1_ + coeffs_.PVX2_ * dfz)
                     * (abs(Vcx) / (epsilon + abs(Vcx))) * coeffs_.LVX_ * coeffs_.LMUX_ * zeta1;
    ...
    double EX      = (coeffs_.PEX1_ + coeffs_.PEX2_ * dfz + coeffs_.PEX3_ * pow(dfz, 2.))
    ...
    // Lateral force
    double SVY      = normalLoad * ((coeffs_.PVY1_ + coeffs_.PVY2_ * dfz) * coeffs_.LVY_ + (coeffs_.PVY3_ + coeffs_.PVY4_ * dfz)
                      * gammaStar * coeffs_.LKYC_) * coeffs_.LMUY_ * zeta2;
    double SHY      = (coeffs_.PHY1_ + coeffs_.PHY2_ * dfz) * coeffs_.LHY_ + coeffs_.PHY3_ * gammaStar * zeta0 + zeta4 - 1.;
    double KYALPHA0 = coeffs_.PKY1_ * Fz0Prime * sin(2. * atan(normalLoad / (coeffs_.PKY2_ * Fz0Prime))) * coeffs_.LYKA_;
    double KYALPHA  = KYALPHA0 * (1. - coeffs_.PKY3_ * pow(gammaStar, 2.)) * zeta3;
    double KYGAMMA0 = (coeffs_.PHY3_ * KYALPHA0 + normalLoad * (coeffs_.PVY3_ + coeffs_.PVY4_ * dfz)) * coeffs_.LKYC_;
    double MUY      = (coeffs_.PDY1_ + coeffs_.PDY2_ * dfz) * (1. - coeffs_.PDY3_ * pow(gammaStar, 2.)) * coeffs_.LMUY_
                      / (1. +  Vs / coeffs_.LONGVL_);
    double ALPHAY   = alphaStar + SHY;
    double CY       = coeffs_.PCY1_ * coeffs_.LCY_;
    double DY       = MUY * normalLoad * zeta2;
    double EY       = (coeffs_.PEY1_ + coeffs_.PEY2_ * dfz) * (1. - (coeffs_.PEY3_ + coeffs_.PEY4_ * gammaStar)
                      * MathTools::sign(ALPHAY)) * coeffs_.LEY_;
    ...
    double DVYKAPPA = MUY * normalLoad * (coeffs_.RVY1_ + coeffs_.RVY2_ * dfz + coeffs_.RVY3_ * gammaStar)
